    ...
    def extract_request_info_from_issue(self, issue_number: int) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                print(f"Found {len(comments)} comments on issue #{issue_number}", file=sys.stderr)
                for comment in comments:
                    comment_body = comment.body or ""
                    print(f"Checking comment (first 100 chars): {comment_body[:100]}", file=sys.stderr)
                    if "Triage Analysis Completed" in comment_body:
                        print("  ✅ Found triage_report artifact", file=sys.stderr)
                        workflow_artifacts.append("triage_report")
                    elif "Implementation Plan Completed" in comment_body:
                        print("  ✅ Found implementation_plan artifact", file=sys.stderr)
                ...
                print(f"Error extracting workflow artifacts: {e}", file=sys.stderr)
